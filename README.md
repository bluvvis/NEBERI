...
Образы с GHCR по тегу `latest` подтягиваются с **`imagePullPolicy: Always`**. В шаблонах деплойментов есть аннотация **`neberi.io/helm-revision`** — при каждом **`helm upgrade`** (в т.ч. без смены values) поды пересоздаются и снова тянут digest `latest`. После зелёного CI выполните **`Deploy-Team11.ps1`** ещё раз.
